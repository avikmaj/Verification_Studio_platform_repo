...
| 14 | Remote regression on Railway | `make: ccache: No such file or directory` / `Error 127`. Verilator bakes `OBJCACHE = ccache` into `verilated.mk` at configure time because ccache existed in the *builder* stage; the runtime stage never installed it | added `ccache` to the runtime image and set `CCACHE_DIR`. Multi-stage Dockerfiles must carry every tool the generated build system references, not just the ones the app imports |
